...
async function handleSubmit() {
  ...
  try {
    const data = await loginApi({ email: form.value.email, password: form.value.password })
...
<template>
  <div
    class="w-full max-w-4xl bg-white rounded-4xl shadow-card grid md:grid-cols-2 overflow-hidden"
  >
    <div class="p-8 md:p-12 flex flex-col justify-center">
      ...
      <p class="text-gray-500 mb-8">
        Visualisasikan jadwal KRS-mu dengan mudah, cepat, dan otomatis di sini.
      </p>
      ...
      <form class="space-y-5" @submit.prevent="handleSubmit">
        ...
        <p v-if="errorMessage" class="text-sm text-app-red">
          {{ errorMessage }}
        </p>
        ...
        <p class="text-center text-sm text-gray-600">
          ...
          <RouterLink
            :to="{ name: 'register' }"
            class="text-app-red font-semibold"
            >Sign Up</RouterLink
          >
        </p>
      </form>
